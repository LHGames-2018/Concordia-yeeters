...
class Agent:
	...
	def check_game_compatibility(self, game):
		...
		print(self.model.get_input_shape_at(0)+ (3,))
		if len(game_output_shape) != len(self.model.get_input_shape_at(0) + (3,)):
	...
	def train(self, game, nb_epoch=1000, batch_size=1, gamma=0.9, epsilon=[1., .1], epsilon_rate=0.5, reset_memory=False, observe=0, checkpoint=None):
		...
		for epoch in range(nb_epoch):
			...
			while not game_over:
				...
				S = S_prime
				if epoch >= observe:
					batch = self.memory.get_batch(model=model, batch_size=1, gamma=0.8)
					if batch:
